...
public:
    Matrica(std::string dat, bool vrsta);
...
Matrica<Tip>::Matrica(std::string dat, bool binarna) {
    if(binarna) {
        std::ifstream input(dat, std::ios::binary);
        if(!input) {
        ...
        try {
    		for (int i = 0; i < br_redova; i++) {
                ...
                for (int j = 0; j < br_kolona; j++) {
                    input.read(reinterpret_cast<char*>(&(elementi[i][j])), sizeof(Tip));
                }
            }
    	} catch(...) {
    		DealocirajMemoriju(elementi, br_redova);
...
template <typename TipEl>
void Matrica<TipEl>::SacuvajUTekstualnuDatoteku(std::string dat) {
    ...
    for (int i = 0; i < br_redova; i++) {
        for (int j = 0; j < br_kolona; j++) {
            if(i == br_redova - 1 && j == br_kolona - 1) {
                output << elementi[i][j]; 
                std::cout << elementi[i][j];
            }
            else if(j == br_kolona - 1) {
              output << elementi[i][j] << std::endl;  
...
template <typename Tip>
void Matrica<Tip>::ObnoviIzTekstualneDatoteke(std::string dat) {
     std::ifstream input(dat);
    if(!input) {
        throw std::logic_error("Trazena datoteka ne postoji");
    }
...
template <typename TipEl>
void Matrica<TipEl>::SacuvajUBinarnuDatoteku(std::string dat) {
    std::ofstream output(dat, std::ios::binary);
    if(!output) {
        throw std::logic_error("Problemi sa upisom u datoteku");
    }
    ...
    for (int i = 0; i < br_redova; i++) {
        for (int j = 0; j < br_kolona; j++) {
            output.write(reinterpret_cast<char*>(&(elementi[i][j])), sizeof(TipEl));
            if(!output) {
                throw std::logic_error("Problemi sa upisom u datoteku");
            }
       }
...
template <typename Tip>
void Matrica<Tip>::ObnoviIzBinarneDatoteke(std::string dat) {
    std::ifstream input(dat, std::ios::binary);
    if(!input) {
        throw std::logic_error("Trazena datoteka ne postoji");
    }
	...
	for (int i(0); i < c.br_redova; i++) {
		for (int j(0); j < c.br_kolona; j++) {
			...
			for (int k(0); k < m1.br_kolona; k++) {
				c.elementi[i][j] += m1.elementi[i][k] * m2.elementi[k][j];
			}
		}
	}
	return c;
...
int main()
{
	...
	std::cin >> m >> n;
	try {
		Matrica<double> a(m, n, 'A');
		std::cout << "Unesi matricu A:\n";
		std::cin >> a;
		/*a.SacuvajUBinarnuDatoteku("PUJDO.DAT");
		a.ObnoviIzBinarneDatoteke("PUJDO.DAT");*/
		a.SacuvajUTekstualnuDatoteku("PUJDO.TXT");
		a.ObnoviIzTekstualneDatoteke("PUJDO.TXT");
		a.SacuvajUTekstualnuDatoteku("PUJDO.TXT");
	} catch(std::bad_alloc) {
