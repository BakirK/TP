...
public:
    ...
    Administracija(const Administracija &a)
    {
        ...
        while (it != a.studenti.end()) {
            this->studenti[it->first] = std::make_shared<Student>( Student(*(it->second)));
            it++;
        }
        auto it2 = a.laptopi.begin();
        while (it2 != a.laptopi.end()) {
            this->laptopi[it2->first] = std::make_shared<Laptop>(Laptop((it2->second)->DajEvidencijskiBroj(), (it2->second)->DajNaziv(), (it2->second)->DajKarakteristike()));
            it2++;
        }
    }
    //ne treba ovo?
    ...
    ~Administracija()
    {
        ...
        while (it != studenti.end()) {
            studenti[it->first] = nullptr;
            it++;
        }
        auto it2 = laptopi.begin();
        while (it2 != laptopi.end()) {
            laptopi[it2->first] = nullptr;
            it2++;
        }
        laptopi.clear();
        studenti.clear();
    ...
    void RegistrirajNovogStudenta (int brojIndeksa, std::string godinaStudija, std::string imeStudenta, std::string adresa, std::string telefon)
    {
        ...
        studenti[brojIndeksa] = std::make_shared<Student>(Student(brojIndeksa, godinaStudija, imeStudenta, adresa, telefon));
    }
    ...
    void RegistrirajNoviLaptop(int ev_broj, std::string naziv, std::string karakteristike)
    {
        ...
        laptopi[ev_broj] = std::make_shared<Laptop>(Laptop(ev_broj, naziv, karakteristike));
    }
...
int main ()
{
    ...
    std::cout << "POSLIJE" << std::endl << std::endl << std::endl;
    etf.IzlistajLaptope();
    return 0;
}
