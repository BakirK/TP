...
p = std::make_shared<double>(2.13);   //pametne nmg delete uradit nego se sami brisu, i ne mogu im dati mem lokaciju neke promjenljive koja nije dinamicki alocirana i ne moze pozivati na nizove vec samo na individualne promjenljive
...
Multiskupovi dozvolajvaju pojavljivanje istog elementa vise puta dok sets ne. Podaci u njima se interno drze sortirano. Nemaju operacije indeksiranja i push _front jer element ili nije ili jeste u skupu te elementi u skupu nemaju poziciju.
...
//ali ako nema polaj tuzla, onda se to polje kreira u novom paru pa cak i kad koristim std::cout << stanovnistvo["Kifino Selo"]; bez dodjele sa =
...
//NAPOMENA: da smo umjesto poakzivaca na dinamick ialocirani niz koordinata koristili vector, svi ovi konstruktori, 
...
+, -, *, /, %,<, <=, >, >=, ==, !=, &, ^, |, && i || //ove nivarne kao prijateljsku funkciju
...
parametra, nazovimo ih i , pri čemu je funktor koji prima dva parametra (nazovimo ih i ). Ova
funkcija vraća kao rezultat novi funktor (nazovimo ga ) koji prima samo jedan parametar (nazovimo
ga y) takav da vrijedi g(y) = (x0, y). Na primjer, ako je “f” neki konkretan funktor, izraz poput
...
/*Naime, poziv “std::less<int>()” kreira funktor (nazovimo ga ) takav da ( , ) ima vrijednost “true”
ako i samo ako je < . Stoga će izraz “std::bind2nd(std::less<int>(), n)” kreirati novi funktor
(nazovimo ga ) takav da je ( ) = ( , ) pri čemu je vrijednost promjenljive “n”, odnosno ( ) ima
vrijednost “true” ako i samo ako je < . Tako kreirani funktor prosljeđuje se funkciji “count if” kao
